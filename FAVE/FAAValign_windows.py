...
def align(wavfile, trs_input, outfile, FADIR='', SOXPATH='/usr/bin/sox', HTKTOOLSPATH=''):
    ...
    print(os.getcwd())
    if not os.path.isdir(os.path.join(os.getcwd(),"tmp"+identifier)):
        print("Making tmp directory")
        os.mkdir(os.path.join(os.getcwd(),"tmp"+identifier))
    #os.listdir(os.getcwd()+'/tmp'+identifier)
    #os.listdir('./tmp'+identifier)
    # prepare wavefile
    SR = prep_wav(wavfile, os.path.join(os.getcwd(),'tmp' + identifier, 'tmp'+identifier+'.wav'), SOXPATH)
    print(SR)

    # prepare mlfile
    prep_mlf(trs_input, os.path.join(os.getcwd(), 'tmp'+ identifier, 'tmp' + identifier + '.mlf'), identifier)
 
    # prepare scp files
    fw = io.open(os.path.join(os.getcwd(),'tmp' + identifier, 'codetr' + identifier + '.scp'), 'w')
    fw.write(os.path.join(os.getcwd(),'tmp' + identifier, 'tmp' + identifier + '.wav') + ' ' + os.path.join(os.getcwd(), 'tmp' + identifier, 'tmp'+ identifier + '.plp') + '\n')
    fw.close()
    fw = io.open(os.path.join(os.getcwd(),'tmp' + identifier, 'test' + identifier + '.scp'), 'w')
    fw.write(os.path.join(os.getcwd(),'tmp' + identifier, 'tmp' + identifier + '.plp') +'\n')
    fw.close()

    try:
        # call plp.sh and align.sh
        if HTKTOOLSPATH:  ## if absolute path to HTK Toolkit is given
            print("Full path given")
            config_file = os.path.join(os.getcwd(),'model',str(SR),'config')
            tmp_scp = os.path.join(os.getcwd(), 'tmp'+identifier,'codetr'+identifier+'.scp')
            blub = os.path.join(os.getcwd(), 'tmp'+identifier, 'blubbeldiblubb.txt')
            os.system(os.path.join(HTKTOOLSPATH,'HCopy')+' -T 1 -C ' + config_file + ' -S '+ tmp_scp + ' >> ' + blub)
            
            mlf_file = os.path.join(os.getcwd(), 'tmp'+identifier, 'tmp'+identifier+'.mlf')
            model_macros = os.path.join(os.getcwd(), 'model', str(SR), 'macros')
            model_hmmdefs = os.path.join(os.getcwd(), 'model', str(SR), 'hmmdefs')
            test_scp = os.path.join(os.getcwd(), 'tmp'+identifier, 'test'+identifier+'.scp')
            aligned_mlf = os.path.join(os.getcwd(), 'tmp'+identifier,'aligned'+identifier+'.mlf')
            monophones = os.path.join(os.getcwd(), 'model', 'monophones')
            aligned_results = os.path.join(os.getcwd(), 'tmp'+identifier, 'aligned'+identifier+'.results')
            
            os.system(os.path.join(HTKTOOLSPATH, 'HVite') +' -T 1 -a -m -I ' + mlf_file + ' -H ' + model_macros + ' -H ' + model_hmmdefs + ' -S ' + test_scp + ' -i ' + aligned_mlf + ' -p 0.0 -s 5.0 ' + options.dict + ' ' + monophones +' > ' + aligned_results)
        else:  ## find path via shell
            print("Finding path via shell")
            ...
            config_file = os.path.join('model',str(SR),'config')
            tmp_scp = os.path.join(os.getcwd(), 'tmp'+identifier,'codetr'+identifier+'.scp')
            blub = os.path.join(os.getcwd(), 'tmp'+identifier, 'blubbeldiblubb.txt')
            os.system('HCopy -T 1 -C ' + config_file + ' -S '+ tmp_scp + ' >> ' + blub)
            ...
            os.system('HVite -T 1 -a -m -I ' + mlf_file + ' -H ' + model_macros + ' -H ' + model_hmmdefs + ' -S ' + test_scp + ' -i ' + aligned_mlf + ' -p 0.0 -s 5.0 ' + options.dict + ' ' + monophones +' > ' + aligned_results)

        ## write result of alignment to TextGrid file
        aligned_to_TextGrid(os.path.join(os.getcwd(), 'tmp' + identifier,'aligned' + identifier + '.mlf'), outfile, SR)
        if options.verbose:
        ...
        #shutil.rmtree(os.path.join(os.getcwd(),"tmp" + identifier))
...
def cut_chunk(wavfile, outfile, start, dur, SOXPATH='/usr/bin/sox'):
...
def prep_wav(orig_wav, out_wav, SOXPATH='/usr/bin/sox'):
...
##        #SR = 11025
    if SOXPATH:  ## if FAAValign is used as a CGI script, the path to SoX needs to be specified explicitly
        soxcmd=SOXPATH + ' \"' + orig_wav + '\" -c 1 -r 16000 ' + out_wav
        print(soxcmd)
        os.system(soxcmd)
...
def FAAValign(opts, args, FADIR='C:\\Users\\Jade\\Documents\\College stuff\\HamLab\\transcription-master\\FAVE\\FAVE-align\\tmp', SOXPATH='/usr/bin/sox'):
